...
router.post('/register', async (req, res) => {
  try {
    ...
    const user = new User({ email, password: hashedPassword, discord });
    await user.save();
...
router.get('/me', async (req, res) => {
  try {
    ...
    res.json({ user: { id: user._id, email: user.email, plan: user.plan, webhook: user.webhook, discord: user.discord } });
  } catch (error) {
